...
test('it can work with multiple providers', function () {
    ...
    $collection = new RoutingProviderCollection($routingProvider1, $routingProvider2, $routingProvider3);
    
    $routingProvider1->shouldReceive('registerRoutes')
        ->with($router)
        ->once();
    $routingProvider2->shouldReceive('registerRoutes')
        ->with($router)
        ->once();
    $routingProvider3->shouldReceive('registerRoutes')
        ->with($router)
        ->once();

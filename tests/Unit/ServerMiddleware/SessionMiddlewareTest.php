...
test('it can process a request and response', function () {
    ...
    $this->processor->shouldReceive('processRequest')
        ->with($request1)
        ->andReturn($request2);
    ...
    $this->processor->shouldReceive('processResponse')
        ->with($request2, $response1)
        ->andReturn($response2);
    
    expect($this->middleware->process($request1, $requestHandler))->toBe($response2);
}); 
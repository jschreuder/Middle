...
test("it can parse json", function () {
    ...
    $body->shouldReceive("getContents")->andReturn(json_encode($array));

    $request1->shouldReceive("getBody")->andReturn($body);
    $request1
...
test("it cannot parse non-array json", function () {
    ...
    $body->shouldReceive("getContents")->andReturn("{invalid,json...code}");
    ...
    $middleware = new JsonRequestParserMiddleware();
    expect(fn() => $middleware->process($request, $requestHandler))->toThrow(
...
test("it cannot parse invalid json", function () {
    ...
    $body->shouldReceive("getContents")->andReturn('"just a string"');

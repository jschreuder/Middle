...
test(
    "it can process a complete request through the full middleware stack",
    function () {
        // Setup router
        ...
        $fallback = CallableController::fromCallable(function () use (
            $fallbackResponse,
        ) {
        ...
        $errorController = CallableController::fromCallable(function (
            $request,
        ) use ($errorResponse) {
        ...
        $sessionProcessor = new class implements SessionProcessorInterface {
            public function processRequest(
                ...
                return $request->withAttribute(
                    "session",
                    new LaminasSession(new SessionManager(), new Container()),
                );
            }
        ...
        $app = $app->withMiddleware(
            new RequestValidatorMiddleware(function ($request, $exception) {
        ...
        $app = $app->withMiddleware(
            new ErrorHandlerMiddleware(new NullLogger(), $errorController),
        );
        ...
        $request = new ServerRequest(
            ...
            "POST",
            new StreamFactory()->createStream('{"name":"  John Doe  "}'),
            [
...
test("it handles routing to fallback when no route matches", function () {
    ...
    $request = new ServerRequest(
        [],
        [],
        new Uri("http://localhost/nonexistent"),
        "GET",
...
test("it handles validation failure through the full stack", function () {
    ...
    $fallback = CallableController::fromCallable(
        fn() => Mockery::mock(ResponseInterface::class),
    );

    $errorController = CallableController::fromCallable(
        fn() => Mockery::mock(ResponseInterface::class),
    );

    $app = new ApplicationStack(new ControllerRunner());
    $app = $app->withMiddleware(
        new RequestValidatorMiddleware(function ($request, $exception) use (
...
test("it passes route parameters to controllers", function () {
    ...
    $controller = CallableController::fromCallable(function (
        ServerRequestInterface $request,
    ) use (&$capturedId, $expectedResponse) {
...
test("it logs all middleware execution through the full stack", function () {
    ...
    $controller = CallableController::fromCallable(function () use (
        $expectedResponse,
    ) {
        return $expectedResponse;
    });
    ...
    $logger
        ...
        ->with(
            Mockery::pattern(
                "/Middleware started: .*JsonRequestParserMiddleware/",
            ),
        )
        ->once()
        ->ordered();
    ...
    $logger
        ...
        ->with(
            Mockery::pattern(
                "/Middleware finished: .*JsonRequestParserMiddleware/",
            ),
        )
        ->once()
        ->ordered();
